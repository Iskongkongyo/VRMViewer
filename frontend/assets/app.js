...
const LOCAL_BACKEND_FALLBACK_BASE = 'http://127.0.0.1:8787';
const BACKEND_BASE_OVERRIDE =
  typeof window.__VRM_DOWNLOAD_BACKEND_BASE__ === 'string'
    ? window.__VRM_DOWNLOAD_BACKEND_BASE__.trim()
    : '';
let resolvedBackendBasePromise = null;
const TRANSLATIONS = {
  en: {
    app: {
    ...
    download: {
      ...
      cookieLabel: 'VRoid Hub Cookie (optional)',
      cookiePlaceholder: 'Cookie: key=value; ...',
      cookieHint: 'A non-empty frontend Cookie overrides the backend Cookie. It stays temporary unless you explicitly click Save Cookie.',
      saveCookie: 'Save Cookie',
      clearSavedCookie: 'Clear saved',
      cookieSaved: 'Cookie saved to this browser.',
      cookieLoaded: 'Loaded the Cookie previously saved in this browser.',
      cookieCleared: 'Saved Cookie cleared. The current input was also cleared.',
      cookieSaveEmpty: 'Cookie is empty. Enter a Cookie before saving.',
      cookieStorageUnavailable: 'Browser storage is unavailable, so the Cookie could not be saved.',
      fetch: 'Fetch VRM',
  ...
  zh: {
    ...
    download: {
      copy: '粘贴 hub.vroid.com 链接。后端会先解混淆，然后本查看器会直接预览并提供 VRM 文件。',
      placeholder: 'https://hub.vroid.com/characters/...',
      cookieLabel: 'VRoid Hub Cookie（可选）',
      cookiePlaceholder: 'Cookie: key=value; ...',
      cookieHint: '非空的前端 Cookie 优先于后端 Cookie。默认仅临时使用，只有主动点击“保存 Cookie”后才会写入当前浏览器。',
      saveCookie: '保存 Cookie',
      clearSavedCookie: '清除已保存',
      cookieSaved: 'Cookie 已保存到当前浏览器。',
      cookieLoaded: '已加载此前保存在当前浏览器中的 Cookie。',
      cookieCleared: '已清除保存的 Cookie，并清空当前输入框。',
      cookieSaveEmpty: 'Cookie 为空，请先填写 Cookie 再保存。',
      cookieStorageUnavailable: '浏览器存储不可用，Cookie 无法保存。',
      fetch: '获取 VRM',
      noticeTitle: '下载提示',
  ...
  ja: {
    ...
    download: {
      ...
      placeholder: 'https://hub.vroid.com/characters/...',
      cookieLabel: 'VRoid Hub Cookie（任意）',
      cookiePlaceholder: 'Cookie: key=value; ...',
      cookieHint: '空でないフロントエンド Cookie はバックエンド Cookie より優先されます。通常は一時利用で、「Cookie を保存」を押した場合のみブラウザに保存されます。',
      saveCookie: 'Cookie を保存',
      clearSavedCookie: '保存を削除',
      cookieSaved: 'Cookie をこのブラウザに保存しました。',
      cookieLoaded: 'このブラウザに保存されていた Cookie を読み込みました。',
      cookieCleared: '保存済み Cookie を削除し、現在の入力欄も空にしました。',
      cookieSaveEmpty: 'Cookie が空です。保存する Cookie を入力してください。',
      cookieStorageUnavailable: 'ブラウザストレージを利用できないため、Cookie を保存できません。',
      fetch: 'VRM を取得',
...
const downloadCookieInput = document.getElementById('download-cookie-input');
const downloadCookieSaveButton = document.getElementById('download-cookie-save-button');
const downloadCookieClearButton = document.getElementById('download-cookie-clear-button');
const downloadCookieStorageStatus = document.getElementById('download-cookie-storage-status');
...
function createVrmLoader() {
  ...
  nextLoader.setWithCredentials(true);
...
function showDownloadResult(result) {
  ...
  downloadResultUrl.textContent = result.displayUrl || stripSensitiveQueryFromUrl(finalUrl);
  downloadFileLink.href = finalUrl;
  downloadFileLink.download = result.fileName || inferFileNameFromUrl(finalUrl);
}

function setDownloadBusy(isBusy) {
  downloadSubmitButton.disabled = isBusy;
  downloadUrlInput.disabled = isBusy;
  downloadCookieInput.disabled = isBusy;
...
function stripSensitiveQueryFromUrl(url) {
  if (!url) {
    return '';
  }
...
function getBackendBaseCandidates() {
  ...
  const currentOrigin = normalizeBackendBase(window.location.origin);

  [BACKEND_BASE_OVERRIDE, queryBackend, currentOrigin, LOCAL_BACKEND_FALLBACK_BASE].forEach((candidate) => {
...
async function resolveBackendBase() {
  if (!resolvedBackendBasePromise) {
    resolvedBackendBasePromise = (async () => {
      const candidates = getBackendBaseCandidates();

      for (const candidate of candidates) {
        try {
          const response = await fetchJson(`${candidate}/api/health`, buildBackendFetchOptions({ method: 'GET' }));
      ...
      resolvedBackendBasePromise = null;
      throw error;
    });
  }

  return resolvedBackendBasePromise;
}

function resolveBackendUrl(url, backendBase) {
...
function normalizeDownloadResult(payload, backendBase) {
  if (!payload || typeof payload !== 'object') {
    return null;
  }

  const directUrl = payload.downloadUrl || payload.modelUrl || '';
  if (directUrl) {
    const finalUrl = resolveBackendUrl(directUrl, backendBase);
    return {
      ...payload,
      downloadUrl: finalUrl,
      modelUrl: finalUrl,
      fileName: payload.fileName || inferFileNameFromUrl(finalUrl),
      displayUrl: stripSensitiveQueryFromUrl(finalUrl),
    };
  }
  ...
  const artifactUrl = artifact?.downloadUrl || artifact?.downloadPath || '';

  if (job?.status === 'done' && artifact?.available && artifactUrl) {
    const finalUrl = resolveBackendUrl(artifactUrl, backendBase);
    return {
      ok: true,
      jobId: job.id || null,
      status: job.status,
      downloadUrl: finalUrl,
      modelUrl: finalUrl,
      fileName: artifact.fileName || inferFileNameFromUrl(finalUrl),
      displayUrl: stripSensitiveQueryFromUrl(finalUrl),
      artifact,
      job,
    };
...
function extractBackendJobPayload(payload) {
  if (!payload || typeof payload !== 'object') {
    return null;
  }
...
function extractBackendJobId(payload) {
  const job = extractBackendJobPayload(payload);
  if (job?.id) {
    return job.id;
  }
...
async function pollJob(jobId, backendBase) {
  ...
  while (Date.now() < deadline) {
    ...
    const response = await fetchJson(
      `${backendBase}/api/jobs/${encodeURIComponent(jobId)}`,
    ...
    const normalizedResult = normalizeDownloadResult(response.data, backendBase);
    if (normalizedResult) {
      return normalizedResult;
    }

    const job = extractBackendJobPayload(response.data);
    if (!job) {
      throw new Error(t('error.pollFailed'));
    }
    ...
    if (job.status === 'done' && job.result) {
      ...
      const legacyResult = normalizeDownloadResult(job.result, backendBase);
      if (legacyResult) {
        return legacyResult;
      }
    ...
    if (job.status === 'error') {
      throw new Error(job.error?.message || job.result?.message || t('error.backendJobFailed'));
    }
...
const DOWNLOAD_COOKIE_STORAGE_KEY = 'vrm-viewer-pro:vroid-hub-cookie';

function normalizeDownloadCookie(value) {
...
function setDownloadCookieStorageStatus(messageKey = '') {
  if (!downloadCookieStorageStatus) {
    return;
  }
  downloadCookieStorageStatus.textContent = messageKey ? t(messageKey) : '';
}

function readSavedDownloadCookie() {
  try {
    return normalizeDownloadCookie(window.localStorage.getItem(DOWNLOAD_COOKIE_STORAGE_KEY) || '');
  } catch {
    return '';
  }
}

function syncDownloadCookieStorageUi() {
  const savedCookie = readSavedDownloadCookie();
  if (downloadCookieClearButton) {
    downloadCookieClearButton.hidden = !savedCookie;
  }
  return savedCookie;
}

function loadSavedDownloadCookie() {
  const savedCookie = syncDownloadCookieStorageUi();
  if (!savedCookie || !downloadCookieInput) {
    return;
  }

  downloadCookieInput.value = savedCookie;
  setDownloadCookieStorageStatus('download.cookieLoaded');
}

function saveDownloadCookie() {
  const cookie = normalizeDownloadCookie(downloadCookieInput?.value || '');
  if (!cookie) {
    setDownloadCookieStorageStatus('download.cookieSaveEmpty');
    return;
  }

  try {
    window.localStorage.setItem(DOWNLOAD_COOKIE_STORAGE_KEY, cookie);
    downloadCookieInput.value = cookie;
    syncDownloadCookieStorageUi();
    setDownloadCookieStorageStatus('download.cookieSaved');
  } catch {
    setDownloadCookieStorageStatus('download.cookieStorageUnavailable');
  }
}

function clearSavedDownloadCookie() {
  try {
    window.localStorage.removeItem(DOWNLOAD_COOKIE_STORAGE_KEY);
  } catch {
    setDownloadCookieStorageStatus('download.cookieStorageUnavailable');
    return;
  }

  if (downloadCookieInput) {
    downloadCookieInput.value = '';
  }
  syncDownloadCookieStorageUi();
  setDownloadCookieStorageStatus('download.cookieCleared');
}

async function requestDownloadedModel(url, cookie = '') {
  const backendBase = await resolveBackendBase();
  const normalizedCookie = normalizeDownloadCookie(cookie);
  const response = await fetchJson(
    `${backendBase}/api/deobfuscate`,
    buildBackendFetchOptions({
      method: 'POST',
      headers: {
  ...
  const jobId = extractBackendJobId(response.data);
  if (jobId) {
    return pollJob(jobId, backendBase);
  }
...
downloadCookieSaveButton?.addEventListener('click', saveDownloadCookie);
downloadCookieClearButton?.addEventListener('click', clearSavedDownloadCookie);
...
downloadForm.addEventListener('submit', async (event) => {
  ...
  try {
    const result = await requestDownloadedModel(validatedUrl, downloadCookieInput.value);
